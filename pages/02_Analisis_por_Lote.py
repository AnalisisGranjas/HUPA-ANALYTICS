    ...
    if not df_full_lote.empty:
        ...
        def render_kpi(col, title, emoji, val, tab, ant_val, unit, css_class, is_mort=False):
            ...
            if tab is not None:
                ...
                html_tab = f'<span class="{status}" style="font-size:1.4rem;">Vs Tab: {dif:+.1f}{unit}</span>'
    
            html_ant = ""
            if ant_val is not None:
                cambio = val - ant_val
                flecha = "▲" if val >= ant_val else "▼"
                html_ant = f'<span style="font-size:1.4rem; opacity:0.9;">Vs Ant: {flecha}{abs(cambio):.1f}</span>'
          ...
          # --- RENDERIZADO CORREGIDO ---
            if title == "SALDO AVES":
                ...
                html_ant = f'<span style="font-size:1.4rem; opacity:0.9;">Encasetadas: {txt_inicial}</span>'
                html_tab = f'<span style="font-size:1.4rem; opacity:0.9;">Mort: {txt_mortalidad}</span>'
            col.markdown(f"""
