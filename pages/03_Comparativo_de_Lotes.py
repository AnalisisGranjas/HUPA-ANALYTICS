...
def load_data():
    if os.path.exists(PATH_DATA):
        try:
            ...
            num_cols = [
                "Edad Sem.", "Saldo de Aves", "Mort", "Suma Mort + Sel",
                "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "Bulto X 40 K",
                "Gr.A.D Real", "Gr.A.D Tabla", "Peso Real", "Peso Tab",
                "Huevos  Semana", "% Pdn. Real", "% Pdn. Tabla", "H.A.A. Real",
                "H.A.A. Tabla", "Jumbo", "Extra", "AA", "A", "B", "C",
                "Alt Cáscara", "Alt. Color", "Picado", "Roto",
                "Costo Alimento Sem", "$ Huevo por alimento", "% Unif"
            ]
...
if df_orig is not None:
    with st.expander("📌 Protocolo de Benchmarking Operativo e Instrucciones", expanded=False):
        st.markdown(
            """
    ...
    # REGLA DE ORO: FILTRAR FILAS DONDE LOTE == NUM_GALPON
    col_galpon = None
    for cg in ["GALPON", "GALPÓN", "NUM_GALPON", "NUM GALPON", "Galpon", "Galpón"]:
        if cg in df_orig.columns:
            col_galpon = cg
            break

    if col_galpon and "LOTE" in df_orig.columns:
        df_orig["LOTE_STR"] = df_orig["LOTE"].astype(str).str.strip()
        df_orig["GALPON_STR"] = df_orig[col_galpon].astype(str).str.strip()
        df_activos = df_orig[df_orig["LOTE_STR"] == df_orig["GALPON_STR"]].copy()
    else:
    ...
    with c1:
        with st.popover("🏢 1. Seleccionar Empresas", use_container_width=True):
            ...
            if col_b1.button("Marcar todo", key="btn_emp_all"):
                for emp in empresas_disp: st.session_state[f"chk_emp_{emp}"] = True
                st.rerun()
            if col_b2.button("Desmarcar", key="btn_emp_none"):
                for emp in empresas_disp: st.session_state[f"chk_emp_{emp}"] = False
                st.rerun()
    ...
    with c2:
        with st.popover(f"🗺️ 2. Granjas ({len(granjas_disp)})", use_container_width=True):
            ...
            if col_b1.button("Marcar todo", key="btn_gr_all"):
                for gr in granjas_disp: st.session_state[f"chk_gr_{gr}"] = True
                st.rerun()
            if col_b2.button("Desmarcar", key="btn_gr_none"):
                for gr in granjas_disp: st.session_state[f"chk_gr_{gr}"] = False
                st.rerun()
    ...
    with c3:
        with st.popover(f"📦 3. Lotes ({len(lotes_disp)})", use_container_width=True):
            ...
            if col_b1.button("Marcar todo", key="btn_lt_all"):
                for lt in lotes_disp: st.session_state[f"chk_lt_{lt}"] = True
                st.rerun()
            if col_b2.button("Desmarcar", key="btn_lt_none"):
                for lt in lotes_disp: st.session_state[f"chk_lt_{lt}"] = False
                st.rerun()
    ...
    opciones_casa = sorted([str(x) for x in df_l["Observaciones"].unique() if pd.notna(x)]) if not df_l.empty else []
    with c4:
        with st.popover("🥣 4. Casa Nutricional", use_container_width=True):
            col_b1, col_b2 = st.columns(2)
            if col_b1.button("Marcar todo", key="btn_cs_all"):
                for cs in opciones_casa: st.session_state[f"chk_cs_{cs}"] = True
                st.rerun()
            if col_b2.button("Desmarcar", key="btn_cs_none"):
                for cs in opciones_casa: st.session_state[f"chk_cs_{cs}"] = False
                st.rerun()
    ...
    with cx2:
        ver_tabla_line = st.toggle("📉 Mostrar línea de Meta Ideal (Tabla)", value=True)

    st.markdown("</div>", unsafe_allow_html=True)

    if not lote_sel or not granja_sel or not empresa_sel:
        st.info("💡 Despliega los desplegables arriba (**Empresas**, **Granja(s)** y **Lote(s)**) y usa el botón **'Marcar todo'** o marca las casillas que deseas comparar.")
    else:
        ...
        # DETECTAR COLUMNA ARCHIVO ORIGEN
        col_archivo = None
        for ca in ["ARCHIVO_ORIGEN", "Archivo_Origen", "archivo_origen", "ARCHIVO", "Archivo", "archivo"]:
            if ca in df_final_base.columns:
                col_archivo = ca
                break

        # PASO 1: EXTRAER 1 SOLA FILA LIMPIA POR GALPÓN/ARCHIVO EN CADA SEMANA (DESDUPLICACIÓN EXACTA DE EXCEL)
        group_dedup_cols = ["ID_INTERNO", "Edad Sem."]
        if col_archivo:
            group_dedup_cols.append(col_archivo)

        df_galpones_limpios = df_final_base.sort_values("Fecha_DT", ascending=True).groupby(group_dedup_cols).last().reset_index()

        # PASO 2: CONSOLIDAR A NIVEL DE LOTE POR SEMANA (SUMA EXACTA Y PROMEDIOS PONDERADOS)
        res_consolidado_list = []
        num_cols_prom = [
            "% Pdn. Real", "% Pdn. Tabla", "Gr.A.D Real", "Gr.A.D Tabla",
            "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "Peso Real", "Peso Tab",
            "H.A.A. Real", "H.A.A. Tabla", "Jumbo", "Extra", "AA", "A", "B", "C",
            "Alt Cáscara", "Alt. Color", "Picado", "Roto", "$ Huevo por alimento"
        ]

        for (id_int, ed_sem), g_sem in df_galpones_limpios.groupby(["ID_INTERNO", "Edad Sem."]):
            row_sem = g_sem.iloc[-1].copy()
            
            # SUMATORIA EXACTA SIN DUPLICADOS
            tot_aves_sem = g_sem["Saldo de Aves"].sum()
            row_sem["Saldo de Aves"] = tot_aves_sem
            row_sem["Huevos  Semana"] = g_sem["Huevos  Semana"].sum() if "Huevos  Semana" in g_sem.columns else 0
            row_sem["Mort"] = g_sem["Mort"].sum() if "Mort" in g_sem.columns else 0
            row_sem["Bulto X 40 K"] = g_sem["Bulto X 40 K"].sum() if "Bulto X 40 K" in g_sem.columns else 0
            row_sem["Costo Alimento Sem"] = g_sem["Costo Alimento Sem"].sum() if "Costo Alimento Sem" in g_sem.columns else 0
            
            # Unificar Fases de Alimento si hay varios galpones
            fases_un = sorted([str(x) for x in g_sem["Fase de Alimento"].unique() if pd.notna(x)])
            row_sem["Fase de Alimento"] = ", ".join(fases_un) if fases_un else "Sin especf."

            # PROMEDIOS PONDERADOS POR AVES DE CADA GALPÓN
            if tot_aves_sem > 0:
                for c in num_cols_prom:
                    if c in g_sem.columns:
                        row_sem[c] = (g_sem[c] * g_sem["Saldo de Aves"]).sum() / tot_aves_sem

            res_consolidado_list.append(row_sem)

        df_final = pd.DataFrame(res_consolidado_list)

        # FORMATEO DE FECHA PARA TOOLTIP
        if "Fecha_DT" in df_final.columns and pd.api.types.is_datetime64_any_dtype(df_final["Fecha_DT"]):
            df_final["Fecha_Fmt"] = df_final["Fecha_DT"].dt.strftime("%d/%b/%Y")
        elif "Final Sem" in df_final.columns:
            df_final["Fecha_Fmt"] = pd.to_datetime(df_final["Final Sem"], errors="coerce").dt.strftime("%d/%b/%Y")
        else:
        ...
        df_final["Conversión"] = (df_final["Bulto X 40 K"] * 40000) / (df_final["Huevos  Semana"].replace(0, 1))
        ...
        for c in ["Grande", "Mediano", "Pequeño", "Segunda", "% Pdn. Real", "Gr.A.D Real", "Peso Real", "H.A.A. Real", "Conversión", "Costo Alimento Sem", "$ Huevo por alimento"]:
            if c in df_graf.columns:
                df_graf[c] = df_graf[c].replace(0, np.nan)

        # MOTOR DE GRÁFICOS COMPARATIVOS LIMPIOS Y CONTINUOS
        def plot_hupa_comparativo(df, y_col, tab_col, title, unit, show_labels, show_table):
            ...
            cdata = np.stack((df_plot["Fecha_Fmt"], df_plot["Fase de Alimento"]), axis=-1)
            ...
            fig.update_traces(
                hovertemplate=(
                    ...
                    "Valor: <b>%{y:.1f} " + unit + "</b><extra></extra>"
                ),
                customdata=cdata,
            )

            fig.update_layout(
                title={"text": f"<b>{title}</b>", "x": 0.5, "xanchor": "center", "yanchor": "top", "font": dict(size=14)},
                height=360,
        ...
        with st.expander("💹 Distribución de Tamaño de Huevo Comerciales", expanded=True):
            g1, g2 = st.columns(2)
            with g1:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Grande", None, "💎 HUEVO GRANDE (Jumbo / Extra / AA) %", "%", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})
            with g2:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Mediano", None, "🥚 HUEVO MEDIANO (A) %", "%", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})

            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)

            g3, g4 = st.columns(2)
            with g3:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Pequeño", None, "🐣 HUEVO PEQUEÑO (B / C) %", "%", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})
            with g4:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Segunda", None, "⚠️ HUEVO DE SEGUNDA %", "%", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})

        # --- 2. SECCIÓN GRÁFICOS: INDICADORES TÉCNICOS ---
        with st.expander("🥚 Comparativo de Indicadores Técnicos Biológicos", expanded=True):
            b1, b2 = st.columns(2)
            with b1:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "% Pdn. Real", "% Pdn. Tabla", "📈 CURVA DE PRODUCCIÓN (%)", "%", ver_etiquetas, ver_tabla_line), use_container_width=True, config={"displayModeBar": False})
            with b2:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Gr.A.D Real", "Gr.A.D Tabla", "🥣 CONSUMO DE ALIMENTO (G/AVE/DÍA)", "g", ver_etiquetas, ver_tabla_line), use_container_width=True, config={"displayModeBar": False})

            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)
            b3, b4 = st.columns(2)
            with b3:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "🛡️ MORTALIDAD ACUMULADA (%)", "%", ver_etiquetas, ver_tabla_line), use_container_width=True, config={"displayModeBar": False})
            with b4:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Peso Real", "Peso Tab", "⚖️ PESO CORPORAL DE LA AVE (G)", "g", ver_etiquetas, ver_tabla_line), use_container_width=True, config={"displayModeBar": False})

        # --- 3. SECCIÓN: EFICIENCIA ALIMENTICIA Y COSTO HUEVO ---
        with st.expander("💰 Eficiencia Alimenticia y Costo por Huevo (con Fase Nutricional)", expanded=True):
            c_ec1, c_ec2 = st.columns(2)
            with c_ec1:
                st.plotly_chart(plot_hupa_comparativo(df_graf, "Conversión", None, "🌾 CONVERSIÓN (G Alimento / Huevo)", "g", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Interpretación:</b> Gramos de alimento consumidos para producir un huevo. Menor valor representa mejor eficiencia biológica.</div>', unsafe_allow_html=True)

            with c_ec2:
                if es_admin:
                    st.plotly_chart(plot_hupa_comparativo(df_graf, "$ Huevo por alimento", None, "🥚 COSTO HUEVO POR ALIMENTO ($/Huevo)", "$", ver_etiquetas, False), use_container_width=True, config={"displayModeBar": False})
                    st.markdown('<div class="guide-box"><b>Interpretación:</b> Dinero invertido en alimento por cada huevo producido.</div>', unsafe_allow_html=True)
                else:
        ...
        with c_tit:
            st.markdown("### 📋 Matriz Detallada de Auditoría Comparativa")
        with c_mod:
            modo_vista = st.selectbox("👁️ Modo de Vista de Tabla", ["Tabla HTML AgroTech", "Visor Tabular Interactivo"], index=0)

        ids = sorted(df_final["ID_INTERNO"].unique())
        tabs = st.tabs([f"📍 {n}" for n in ids])

        cols_base = [
            "Final Sem", "Edad Sem.", "Saldo de Aves", "Mort", "% Mort + Sel Acum.",
            "%Mort+Sel Acum. Tab", "Dif Mort", "Fase de Alimento", "Observaciones",
            "Gr.A.D Real", "Gr.A.D Tabla", "Dif GAD", "% Unif", "Peso Real",
            "Peso Tab", "Dif Peso", "Huevos  Semana", "% Pdn. Real", "% Pdn. Tabla",
            "Dif Pdn", "H.A.A. Real", "H.A.A. Tabla", "Dif HAA", "Conversión",
            "Grande", "Mediano", "Pequeño", "Segunda", "Costo Alimento Sem", "$ Huevo por alimento",
        ]
        ...
        for i, tab in enumerate(tabs):
            with tab:
                ...
                genetica_lote = df_tab["LINEA_AVES"].iloc[0] if "LINEA_AVES" in df_tab.columns and not df_tab.empty else "N/A"
                st.markdown(f'#### 🧬 Línea Genética: <span class="genetica-badge">{genetica_lote}</span>', unsafe_allow_html=True)
                ...
                # TABLA DESGLOSADA POR GALPÓN LIMPIA
                tabla_render = df_tab[cols_disp].sort_values("Edad Sem.", ascending=False).fillna(0).copy()

                if "Final Sem" in tabla_render.columns:
                    tabla_render["Final Sem"] = pd.to_datetime(tabla_render["Final Sem"], errors="coerce").dt.strftime("%d/%m/%y")
                ...
                if modo_vista == "Tabla HTML AgroTech":
                        ...
                        for idx, row in df_data.iterrows():
                            ...
                            for col in df_data.columns:
                                ...
                                if col == "Dif Pdn" and pd.notnull(val):
                                    try:
                                        n_v = float(val)
                                        style = "background-color:rgba(17,122,101,0.18); color:#117A65; font-weight:bold;" if n_v >= 0 else "background-color:rgba(169,50,38,0.18); color:#A93226; font-weight:bold;"
                                    except: pass

                                elif col == "Dif GAD" and pd.notnull(val):
                                    try:
                                        n_v = float(val)
                                        style = "background-color:rgba(185,119,14,0.18); color:#B9770E; font-weight:bold;" if n_v > 0 else "background-color:rgba(17,122,101,0.18); color:#117A65; font-weight:bold;"
                                    except: pass

                                elif col == "Dif Mort" and pd.notnull(val):
                                    try:
                                        n_v = float(val)
                                        style = "background-color:rgba(169,50,38,0.18); color:#A93226; font-weight:bold;" if n_v < 0 else "background-color:rgba(17,122,101,0.18); color:#117A65; font-weight:bold;"
                                    except: pass

                                if col in formatos_activos and pd.notnull(val) and isinstance(val, (int, float)):
                    ...
                    for col, fmt in formatos_activos.items():
                        if col in df_interactivo.columns:
                            df_interactivo[col] = df_interactivo[col].apply(lambda x: fmt.format(x) if pd.notnull(x) else "")

                    st.dataframe(df_interactivo, use_container_width=True, hide_index=True)
