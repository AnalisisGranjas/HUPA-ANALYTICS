...
st.markdown(
    ...
    /* PANEL DE FILTROS CERRADO Y ESTRUCTURADO */
    .filter-panel {
...
st.markdown(
    ...
    <div class="app-header">
        ...
        <div class="app-header-text">
            ...
            <p>Balance táctico, desempeño biológico y auditoría ejecutiva de lotes de postura</p>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
if df_orig is not None:
    ...
    with st.expander("📌 Balance Táctico e Instrucciones del Tablero", expanded=False):
        st.markdown(
            """
    ...
    # Fila 1: Empresa y Modo de Vista
    ...
    with c_emp:
        empresa = st.selectbox("🏢 Empresa / Razón Social:", empresas_disp, index=idx_default)
    ...
    # Lógica "Foto Actual" de lotes activos
    df_activos = df_empresa[df_empresa["LOTE"] == df_empresa["NUM_GALPON"]].copy()
    df_sorted = df_activos.sort_values(
        ["GRANJA", "LOTE", "Edad Sem."], ascending=[True, True, False]
    )
    df_ultimos_base = df_sorted.groupby(["GRANJA", "LOTE"]).nth(1).reset_index()
    ...
    lista_granjas = ["Todas"] + sorted([str(x) for x in df_ultimos_base["GRANJA"].unique() if pd.notnull(x)])
    lista_genetica = ["Todas"] + sorted([str(x) for x in df_ultimos_base["LINEA_AVES"].unique() if pd.notnull(x)])
    lista_alimento = ["Todas"] + sorted([str(x) for x in df_ultimos_base["Fase de Alimento"].unique() if pd.notnull(x)])

    with f_col1:
        sel_granja = st.selectbox("🏘️ Filtrar Granja:", lista_granjas, index=0)
    with f_col2:
        sel_genetica = st.selectbox("🧬 Filtrar Genética:", lista_genetica, index=0)
    with f_col3:
        sel_alimento = st.selectbox("🥣 Filtrar Fase de Alimento:", lista_alimento, index=0)

    # Fila 3: Slider Rango de Edad
    edad_min = int(df_ultimos_base["Edad Sem."].min()) if not df_ultimos_base.empty else 18
    edad_max = int(df_ultimos_base["Edad Sem."].max()) if not df_ultimos_base.empty else 100
    ...
    if sel_granja != "Todas":
        df_ultimos = df_ultimos[df_ultimos["GRANJA"].astype(str) == sel_granja]
    if sel_genetica != "Todas":
        df_ultimos = df_ultimos[df_ultimos["LINEA_AVES"].astype(str) == sel_genetica]
    if sel_alimento != "Todas":
        df_ultimos = df_ultimos[df_ultimos["Fase de Alimento"].astype(str) == sel_alimento]
    ...
    with col1:
        st.markdown(
            f"""<div class="kpi-card-exec">
                <div class="kpi-title-exec">🐔 Aves Totales</div>
    ...
    with col2:
        st.markdown(
            f"""<div class="kpi-card-exec">
                <div class="kpi-title-exec">📦 Lotes Activos</div>
                <div class="kpi-val-exec">{len(df_ultimos)}</div>
            </div>""",
            unsafe_allow_html=True,
        )
    with col3:
        val_edad_m = df_ultimos["Edad Sem."].mean() if not df_ultimos.empty else 0
        st.markdown(
            f"""<div class="kpi-card-exec">
    ...
    with col4:
        val_pdn_m = df_ultimos["% Pdn. Real"].mean() if not df_ultimos.empty else 0
        st.markdown(
            f"""<div class="kpi-card-exec">
    ...
    with col5:
        val_mort_m = df_ultimos["% Mort + Sel Acum."].mean() if not df_ultimos.empty else 0
        st.markdown(
            f"""<div class="kpi-card-exec">
    ...
    # --- 🚨 NUEVO BLOQUE: ALERTAS TEMPRANAS (LOTES EN OBSERVACIÓN PLEGABLE) ---
    ...
    titulo_expander_alertas = f"🚨 Alertas Tempranas Operativas ({cant_alertas} Desviaciones Detectadas)" if cant_alertas > 0 else "✅ Diagnóstico de Alertas Operativas (Sin Desviaciones Críticas)"

    with st.expander(titulo_expander_alertas, expanded=False):
        if cant_alertas == 0:
            st.success("🎉 Todos los lotes en la vista actual se encuentran operando dentro de los parámetros biológicos esperados.")
        else:
            col_a1, col_a2 = st.columns(2)
            with col_a1:
                st.markdown("#### 🔴 Desviación Crítica en Postura (<-3% vs Meta)")
                    ...
                    for _, row in lotes_alerta_pdn.iterrows():
                        st.markdown(
                            ...
                            <div class="alert-card-item">
                                <div class="alert-card-title">Granja {row['GRANJA']} - Galpón/Lote {row['LOTE']} ({row['LINEA_AVES']})</div>
                ...
                if lotes_alerta_mort.empty:
                    ...
                    for _, row in lotes_alerta_mort.iterrows():
                        st.markdown(
                            f"""
                            <div class="alert-card-item">
                                <div class="alert-card-title">Granja {row['GRANJA']} - Galpón/Lote {row['LOTE']} (Edad: {row['Edad Sem.']} sem)</div>
                                <div class="alert-card-desc">
    ...
    with st.expander("📊 Ver Gráficos de Distribución de Aves", expanded=True):
        ...
        with cg1:
            st.markdown("<p style='text-align:center; font-weight:bold;'>🗺️ Distribución por Granja</p>", unsafe_allow_html=True)
            if not df_ultimos.empty:
                df_g1 = df_ultimos.groupby("GRANJA", as_index=False)["Saldo de Aves"].sum()
                fig1 = px.pie(df_g1, values="Saldo de Aves", names="GRANJA", hole=0.4, color_discrete_sequence=px.colors.sequential.Oranges_r)
                st.plotly_chart(clean_fig(fig1), use_container_width=True, config={"displayModeBar": False})
            else:
                st.info("Sin datos para los filtros seleccionados.")

        with cg2:
            st.markdown("<p style='text-align:center; font-weight:bold;'>🧬 Distribución por Genética</p>", unsafe_allow_html=True)
            if not df_ultimos.empty:
                df_g2 = df_ultimos.groupby("LINEA_AVES", as_index=False)["Saldo de Aves"].sum()
                fig2 = px.pie(df_g2, values="Saldo de Aves", names="LINEA_AVES", hole=0.4, color_discrete_sequence=px.colors.sequential.YlOrRd_r)
                st.plotly_chart(clean_fig(fig2), use_container_width=True, config={"displayModeBar": False})
            else:
                st.info("Sin datos para los filtros seleccionados.")

        with cg3:
            st.markdown("<p style='text-align:center; font-weight:bold;'>🥣 Distribución por Fase Alimento</p>", unsafe_allow_html=True)
            if not df_ultimos.empty:
                df_g3 = df_ultimos.groupby("Fase de Alimento", as_index=False)["Saldo de Aves"].sum()
                fig3 = px.pie(df_g3, values="Saldo de Aves", names="Fase de Alimento", hole=0.4, color_discrete_sequence=px.colors.sequential.Peach_r)
    ...
    tabla_base = df_ultimos[cols_disponibles].sort_values("Edad Sem.", ascending=False).copy()
    formatos_activos = {k: v for k, v in formatos.items() if k in tabla_base.columns}
    ...
    with c_head2:
        with st.popover("⚙️ Seleccionar Columnas Visibles"):
            cols_visibles = [col for col in cols_disponibles if st.checkbox(col, value=True, key=f"vis_{col}")]
        ...
        if nombre_persona and cols_exportar:
            ...
            for col in cols_exportar:
                if col == cols_exportar[0]:
                    ...
                    "Edad Sem.", "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "Dif Mort",
            ...
            df_reporte_num = pd.concat([tabla_base[cols_exportar], pd.DataFrame([resumen_data])[cols_exportar]], ignore_index=True)
            ...
            for col, fmt in formatos.items():
                if col in df_descarga.columns:
                    df_descarga[col] = df_descarga[col].apply(
                        lambda x: fmt.format(x) if (pd.notnull(x) and not isinstance(x, str)) else x
                    )
            ...
            st.info("💡 Ingrese el nombre del auditor para habilitar el botón de descarga.")
    ...
    if modo_vista == "Tabla HTML AgroTech":
        def render_custom_table(df_data):
                ...
                for col in df_data.columns:
                    ...
                    if col == "Dif Pdn" and pd.notnull(val):
                        try:
                            n_v = float(val)
                            if n_v >= 0:
                                style = "background-color:#E8F8F5; color:#117A65; font-weight:bold;"
                                title_tip = f"🟢 +{n_v:.1f}% sobre la meta"
                            else:
                                style = "background-color:#FDEDEC; color:#A93226; font-weight:bold;"
                        ...
                        try:
                            ...
                            if n_v >= 0:
                                style = "background-color:#E8F8F5; color:#117A65; font-weight:bold;"
                                title_tip = f"🟢 +{n_v:.1f} huevos/ave alojada sobre meta"
                            else:
                                style = "background-color:#FDEDEC; color:#A93226; font-weight:bold;"
                                title_tip = f"🔴 {n_v:.1f} huevos/ave alojada bajo meta"
                        except: pass
                        ...
                        try:
                            n_v = float(val)
                            if n_v > 1.5:
                                style = "background-color:#FEF5E7; color:#B9770E; font-weight:bold;"
                        ...
                        try:
                            ...
                            if n_v < 0:
                                style = "background-color:#FDEDEC; color:#A93226; font-weight:bold;"
                                title_tip = f"🔴 Mortalidad superior a la guía"
                            else:
                                style = "background-color:#E8F8F5; color:#117A65; font-weight:bold;"
                                title_tip = f"🟢 Mortalidad dentro de meta"
                        except: pass
                    ...
                    if style:
                        html += f'<td style="{style}" title="{title_tip}">{fmt_val}</td>'
                    else:
            ...
            for col in df_data.columns:
                if col == df_data.columns[0]:
                    ...
                    val_tot = formatos_activos.get(col, "{:,.0f}").format(df_data[col].sum())
                elif col in [
                    "Edad Sem.", "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "Dif Mort",
                    "$ Huevo por alimento", "Gr.A.D Real", "Gr.A.D Tabla", "Dif GAD",
                    "% Pdn. Real", "% Pdn. Tabla", "Dif Pdn", "H.A.A. Real", "H.A.A. Tabla", "Dif HAA",
                ]:
                    val_tot = formatos_activos.get(col, "{:.1f}").format(df_data[col].mean())
                else:
                    val_tot = ""
        ...
        if "Final Sem" in df_interactivo.columns:
            df_interactivo["Final Sem"] = pd.to_datetime(df_interactivo["Final Sem"]).dt.strftime("%d/%m/%y")
