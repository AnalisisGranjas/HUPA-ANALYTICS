...
import plotly.graph_objects as go # <--- AÑADIMOS ESTO PARA LA LÍNEA IA
import numpy as np
import os
import datetime
...
def obtener_prediccion_ia(df_historico, columna_y, semanas_futuras=3):
    try:
        # Filtramos para analizar solo la fase de persistencia si el lote ya es maduro
        if df_historico['Edad Sem.'].max() >= 30:
            df_entrenamiento = df_historico[df_historico['Edad Sem.'] >= 30].copy()
        else:
            df_entrenamiento = df_historico.copy() # Si es joven, usamos lo que hay

        # Seguro anticrashes: necesitamos al menos 3 semanas para trazar curva
        if len(df_entrenamiento) < 3:
            return None, None

        # Preparamos los datos
        X = df_entrenamiento[['Edad Sem.']].values
        y = df_entrenamiento[columna_y].values
        
        # Matemáticas (Polinomio Grado 2 para suavidad biológica)
        ...
        ultima_sem = df_historico['Edad Sem.'].max()
        X_futuro = np.array([[ultima_sem + i] for i in range(1, semanas_futuras + 1)])
        X_total = np.vstack([X, X_futuro])
        
        # Predecimos
        X_total_poly = poly.transform(X_total)
        y_pred = model.predict(X_total_poly)
        
        return X_total.flatten(), y_pred
        ...
        def plot_hupa_final(df_current, df_total, real_col, tab_col, title, unit):
            ...
            if tab_col and tab_col in df_plot.columns:
                fig.update_traces(
                    ...
                    mode=modo_visual, # Añadimos +text
            ...
            if ver_ia and ML_AVAILABLE:
                ...
                if semanas_ia is not None:
                    # --- AQUÍ ESTÁ EL CAMBIO ---
                    # Cortamos la línea de la IA para que empiece donde empieza tu slider
                    ...
                    fig.add_trace(go.Scatter(
                        x=semanas_filtradas, # Usamos las filtradas
                        y=valores_filtrados, # Usamos los filtrados
                        name="🤖 Tendencia IA",
...
# --- TABLA TÉCNICA MAESTRA ---
        ...
        with st.expander("⚙️ Personalizar columnas visibles"):
            columnas_visibles = st.multiselect(
                "Selecciona o elimina las columnas que deseas analizar:", 
                options=cols_disponibles, 
                default=cols_disponibles,
                label_visibility="collapsed" # Oculta el título duplicado para que se vea más limpio
            )

        # 4. CREAMOS LA TABLA SOLO CON LAS COLUMNAS QUE ELEGISTE
        # Si no elige ninguna, evitamos error mostrando al menos la Edad
        if not columnas_visibles:
            columnas_visibles = ['Edad Sem.'] 
        ...
        # ... (Tu código de formatos, style_diferencias, y el IF de modo_proyector siguen igual) ...
...
# 1. ORDEN PARA PANDAS: Ocultar el índice en el estilo
        tabla_estilizada = tabla_final.style \
            .format({k: v for k, v in formatos.items() if k in tabla_final.columns}) \
            .map(style_diferencias, subset=[c for c in ['Dif Pdn', 'Dif GAD', 'Dif HAA', 'Dif Peso', 'Dif Mort'] if c in tabla_final.columns]) \
            .hide(axis="index") # <--- PRIMERA LLAVE (Oculta en Pandas)

        # 2. ORDEN PARA STREAMLIT: Forzar ocultamiento en la interfaz
        if modo_proyector:
            # MODO PRESENTACIÓN (st.table)
            st.markdown("""
                <style>
                .stTable td { font-size: 20px !important; text-align: center !important; }
                .stTable th { font-size: 22px !important; background-color: #1a237e !important; color: white !important; text-align: center !important; }
                .stTable tbody th { display: none; }
                .stTable thead th:first-child { display: none; }
                </style>
            """, unsafe_allow_html=True)
            st.table(tabla_estilizada)
        else:
            # MODO AUDITORÍA (st.dataframe)
            # Agregar hide_index=True aquí es vital
            st.dataframe(tabla_estilizada, use_container_width=True, hide_index=True) # <--- SEGUNDA LLAVE (Oculta en Streamlit)

        # --- SELLO DE ACTUALIZACIÓN (Debajo de la tabla) ---
        try:
            # Obtenemos la fecha de modificación del archivo Excel
