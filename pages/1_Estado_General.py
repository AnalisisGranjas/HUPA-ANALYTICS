...
st.markdown("""
    ...
    .card-mort { background: linear-gradient(135deg, #c62828, #e53935); } /* Rojo HUPA */        
...
if df_orig is not None:
    ...
    col1.markdown(f'<div class="card-design card-total"><span class="label-card">Encasetamiento Total</span><span class="val-num">🐔 {df_ultimos["Saldo de Aves"].sum():,.0f}</span></div>', unsafe_allow_html=True)
    ...
    col5.markdown(f'<div class="card-design card-mort"><span class="label-card">Mortalidad Prom.</span><span class="val-num">⚰️ {df_ultimos["% Mort + Sel Acum."].mean():.1f}%</span></div>', unsafe_allow_html=True)
    ...
    def clean_fig(fig):
        fig.update_layout(
            ...
            height=320, # Altura ideal para que no ocupe toda la pantalla
            showlegend=False,
