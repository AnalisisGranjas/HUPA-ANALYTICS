...
if df_orig is not None:
    ...
    st.markdown("""
        <div class="intro-card">
            <h4 style="color: #0d47a1; margin-top:0;">🔎 ¿Cómo funciona este comparador?</h4>
            <p style="font-size: 1rem; line-height: 1.5;">
                Aquí comparamos el desempeño de tus lotes seleccionados. La <b>Meta Ideal</b> (línea negra punteada) es tu objetivo basado en la genética; 
                si tus líneas de colores están por encima en producción, ¡vas por excelente camino!
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with cx1: ver_etiquetas = st.toggle("🏷️ Mostrar números en los puntos", value=False)
    with cx2: ver_tabla_line = st.toggle("📉 Mostrar la Meta Ideal (Referencia)", value=True)
    ...
    if not lote_sel:
        ...
        with st.expander("🛠️ Ajustes Avanzados"):
            ...
            with fa1:
                genetica_sel = st.multiselect("🧬 Tipo de Gallina:", sorted(df_f['LINEA_AVES'].unique()), default=df_f['LINEA_AVES'].unique())
            with fa2:
                min_e, max_e = int(df_f['Edad Sem.'].min()), int(df_f['Edad Sem.'].max())
                rango_edad = st.slider("⏳ Rango de Edad (Semanas):", min_e, max_e, (max(min_e, max_e-3), max_e))
        ...
        g1, g2 = st.columns(2)
        with g1:
            st.plotly_chart(plot_hupa(df_graf, 'Grande', None, "💎 GRUPO GRANDE (Jumbo a AA) %", "%", ver_etiquetas, False), use_container_width=True)
            st.markdown('<div class="guide-box"><b>Explicación:</b> Huevos de mayor tamaño y valor comercial. Si esta línea sube, mejora el ingreso por huevo.</div>', unsafe_allow_html=True)
        with g2:
            st.plotly_chart(plot_hupa(df_graf, 'Mediano', None, "🥚 GRUPO MEDIANO (Tipo A) %", "%", ver_etiquetas, False), use_container_width=True)
            st.markdown('<div class="guide-box"><b>Explicación:</b> El tamaño estándar de mercado. Predomina al inicio del ciclo productivo.</div>', unsafe_allow_html=True)
        
        st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True) # ARREGLADO: Ahora se renderiza como HTML
        
        g3, g4 = st.columns(2)
        with g3:
            st.plotly_chart(plot_hupa(df_graf, 'Pequeño', None, "🐣 GRUPO PEQUEÑO (Tipo B y C) %", "%", ver_etiquetas, False), use_container_width=True)
            st.markdown('<div class="guide-box"><b>Explicación:</b> Huevos pequeños. Si persiste en aves adultas, indica problemas de peso o nutrición.</div>', unsafe_allow_html=True)
        with g4:
            st.plotly_chart(plot_hupa(df_graf, 'Segunda', None, "⚠️ HUEVO DE SEGUNDA %", "%", ver_etiquetas, False), use_container_width=True)
            st.markdown('<div class="guide-box"><b>Explicación:</b> Merma (roto, picado, sucio). Debe estar lo más cerca posible de 0%.</div>', unsafe_allow_html=True)

        st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-header">🥚 INDICADORES DE PRODUCCIÓN Y SALUD</div>', unsafe_allow_html=True)
        b1, b2 = st.columns(2)
        with b1:
            st.plotly_chart(plot_hupa(df_graf, '% Pdn. Real', '% Pdn. Tabla', "📈 PRODUCCIÓN TOTAL DE HUEVO (%)", "%", ver_etiquetas, ver_tabla_line), use_container_width=True)
        with b2:
            st.plotly_chart(plot_hupa(df_graf, 'Gr.A.D Real', 'Gr.A.D Tabla', "🥣 ALIMENTO CONSUMIDO (GRAMOS)", "g", ver_etiquetas, ver_tabla_line), use_container_width=True)
        
        st.markdown('---')
        b3, b4 = st.columns(2)
        with b3:
            st.plotly_chart(plot_hupa(df_graf, '% Mort + Sel Acum.', '%Mort+Sel Acum. Tab', "🛡️ VIDAS SALVADAS / VIABILIDAD (%)", "%", ver_etiquetas, ver_tabla_line), use_container_width=True)
        with b4:
            st.plotly_chart(plot_hupa(df_graf, 'Peso Real', 'Peso Tab', "⚖️ PESO CORPORAL DE LA GALLINA (GRAMOS)", "g", ver_etiquetas, ver_tabla_line), use_container_width=True)
