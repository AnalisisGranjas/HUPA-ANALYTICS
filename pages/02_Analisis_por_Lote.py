...
if df_orig is not None:
    ...
    # --- BLOQUE INTRODUCTORIO: AUDITORÍA DE OPERATIVIDAD DEL LOTE ---
    st.markdown("""
    ...
    if not df_full_lote.empty:
        ...
        with g1: 
            st.plotly_chart(plot_hupa_final(df_lote, df_full_lote, '% Pdn. Real', '% Pdn. Tabla', "🥚 PRODUCCION (%)", "%"), use_container_width=True)
        ...
        with g3: 
            st.plotly_chart(plot_hupa_final(df_lote, df_full_lote, '% Mort + Sel Acum.', '%Mort+Sel Acum. Tab', "⚰️ MORTALIDAD ACUM (%)", "%"), use_container_width=True)
        ...
        with g5: 
            st.plotly_chart(plot_hupa_final(df_lote, df_full_lote, 'H.A.A. Real', 'H.A.A. Tabla', "🧑‍🌾 HUEVOS AVE ALOJADA", ""), use_container_width=True)
