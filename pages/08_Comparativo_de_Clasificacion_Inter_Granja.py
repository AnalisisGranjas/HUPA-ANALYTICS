...
def load_data():
    if os.path.exists(PATH_DATA):
        try:
            ...
            # Normalizar Empresa
            col_empresa = next((c for c in ["EMPRESA", "RAZON_SOCIAL", "RAZON SOCIAL", "SOCIEDAD", "Razon Social", "Razón Social", "RS"] if c in df.columns), None)
            df["RAZON_SOCIAL"] = df[col_empresa].astype(str).str.strip() if col_empresa else "HUPA | DIVISIÓN AVÍCOLA"

            # Normalizar Genética
            col_linea = next((cl for cl in ["LINEA_AVES", "LINEA AVES", "LINEA GENETICA", "LINEA_GENETICA", "GENETICA", "LINEA", "Línea Genética", "RAZA", "Raza"] if cl in df.columns), None)
            df["LINEA_AVES"] = df[col_linea].fillna("Sin Especificar").astype(str).str.strip() if col_linea else "Sin Especificar"

            # Normalizar Galpón
            col_galpon = next((cg for cg in ["GALPON", "GALPÓN", "NUM_GALPON", "NUM GALPON", "Galpon", "Galpón"] if cg in df.columns), None)

            # FILTRO DE ORO DE DEDUPLICACIÓN: LOTE == NUM_GALPON
            ...
            if "Observaciones" in df.columns:
                df["Observaciones"] = df["Observaciones"].astype(str).replace(["nan", "None", "", "0", "NaN"], "Sin especificar")
            else:
                df["Observaciones"] = "Sin especificar"
...
if df_orig is not None:
    ...
    empresas_totales = sorted(df_activos["RAZON_SOCIAL"].unique())

    # 1. POPOVER MULTI-RAZÓN SOCIAL
    with f_col1:
        st.markdown("<label style='font-size:14px; font-weight:400; margin-bottom:4px; display:block;'>🏢 1. Razón Social / Empresa:</label>", unsafe_allow_html=True)
        for emp in empresas_totales:
            if f"chk_clas_emp_{emp}" not in st.session_state:
                st.session_state[f"chk_clas_emp_{emp}"] = True

        empresas_sel = [emp for emp in empresas_totales if st.session_state.get(f"chk_clas_emp_{emp}", True)]
        
        with st.popover(f"Empresas Seleccionadas ({len(empresas_sel)}/{len(empresas_totales)})", use_container_width=True):
            b_e1, b_e2 = st.columns(2)
            if b_e1.button("Marcar todo", key="btn_cemp_all"):
                for emp in empresas_totales: st.session_state[f"chk_clas_emp_{emp}"] = True
                st.rerun()
            if b_e2.button("Desmarcar", key="btn_cemp_none"):
                for emp in empresas_totales: st.session_state[f"chk_clas_emp_{emp}"] = False
                st.rerun()

            empresas_sel = [
                emp for emp in empresas_totales
                if st.checkbox(f"{emp}", value=st.session_state.get(f"chk_clas_emp_{emp}", True), key=f"chk_clas_emp_{emp}")
            ]

    df_emp = df_activos[df_activos["RAZON_SOCIAL"].isin(empresas_sel)].copy()
    granjas_totales = sorted(df_emp["GRANJA"].unique())

    # 2. POPOVER MULTI-GRANJA
    with f_col2:
        st.markdown("<label style='font-size:14px; font-weight:400; margin-bottom:4px; display:block;'>🗺️ 2. Granjas a Comparar:</label>", unsafe_allow_html=True)
    ...
    df_g = df_emp[df_emp["GRANJA"].isin(granjas_sel)].copy()
    lotes_totales = sorted(df_g["LOTE"].unique())

    # 3. POPOVER MULTI-LOTE
    with f_col3:
        st.markdown("<label style='font-size:14px; font-weight:400; margin-bottom:4px; display:block;'>📦 3. Lotes Biológicos:</label>", unsafe_allow_html=True)
    ...
    df_comp_raw = df_g[df_g["LOTE"].isin(lotes_sel)].copy()
    ...
    if not empresas_sel or not granjas_sel or not lotes_sel or df_comp_raw.empty:
        st.info("💡 Por favor selecciona al menos una empresa, granja y lote en los filtros superiores.")
    else:
        # --- CONSOLIDACIÓN PONDERADA DE LOTE COMPLETO ---
        res_clas_comp = []

        for (g_nombre, lt_id, ed_sem), sem_df in df_comp_raw.groupby(["GRANJA", "LOTE", "Edad Sem."]):
            ...
            row_sem["GRANJA"] = g_nombre
            row_sem["LOTE"] = lt_id
            row_sem["Edad Sem."] = ed_sem
            ...
            res_clas_comp.append(row_sem)

        df_comp = pd.DataFrame(res_clas_comp).sort_values(by=["GRANJA", "LOTE", "Edad Sem."])

        # --- RESGUARDO DE CENSO BIOLÓGICO CONTINUO (Forward Fill) ---
        resguardados_list = []
        for (g_nombre, lt_id), lote_df in df_comp.groupby(["GRANJA", "LOTE"]):
            lote_df = lote_df.sort_values("Edad Sem.").copy()
            
            # Detectar el censo real estable (máximo razonable del lote)
            max_aves_lote = lote_df["Saldo de Aves"].max()
            
            if max_aves_lote > 0:
                # Si una semana registra caída atípica (por ej. falta de reporte de un galpón), reajustar con el saldo biológico real
                lote_df["Saldo de Aves"] = np.where(
                    lote_df["Saldo de Aves"] < (max_aves_lote * 0.70),
                    max_aves_lote,
                    lote_df["Saldo de Aves"]
                )
            resguardados_list.append(lote_df)

        df_comp = pd.concat(resguardados_list, ignore_index=True)
        ...
        if df_view.empty:
            st.warning("⚠️ No hay registros de clasificación dentro del rango de edad seleccionado.")
        else:
            ...
            # FILTRO DE VALIDACIÓN DE DATOS REALES
            df_valid_clas = df_view[(df_view["Grande %"] + df_view["Mediano %"] + df_view["Pequeño %"]) > 0]

            if not df_valid_clas.empty:
                prom_grande = df_valid_clas["Grande %"].mean()
                prom_mediano = df_valid_clas["Mediano %"].mean()
                prom_pequeno = df_valid_clas["Pequeño %"].mean()
                prom_blanco = df_valid_clas["Blanco %"].mean()
                prom_segunda = df_valid_clas["Segunda %"].mean()
            else:
                prom_grande = prom_mediano = prom_pequeno = prom_blanco = prom_segunda = 0.0
            ...
            # --- SECCIÓN GRÁFICOS CON TOOLTIPS CORREGIDOS ---
            ...
            with st.expander("📊 Histograma de Distribución Acumulada de Gramaje", expanded=True):
                ...
                if not df_valid_clas.empty:
                    df_histo = df_valid_clas.groupby("Identificador")[cat_cols].mean().reset_index()
                    ...
                    fig_histo.update_traces(
                        hovertemplate="<b>%{x}</b><br>%{fullData.name}<br>Participación: <b>%{y:.1f}%</b><extra></extra>"
                    )
                    ...
                    st.info("💡 En la ventana de tiempo seleccionada, ninguno de los lotes cuenta con datos de clasificación registrados.")

            if not df_valid_clas.empty:
                with st.expander("📈 Historial Semanal Completo por Categoría de Huevo", expanded=True):
                    df_sorted = df_valid_clas.sort_values(["Identificador", "Edad Sem."]).copy()
                    cdata = np.stack(
                        (df_sorted["Fecha_Fmt"], df_sorted["Identificador"]), axis=-1
                    )
                    ...
                    def crear_fig_historia(df_data, y_col, titulo_grafico):
                        ...
                        fig.update_traces(
                            ...
                            hovertemplate=(
                                ...
                                "Participación: <b>%{y:.1f}%</b><extra></extra>"
                            ),
                            customdata=cdata
                        )
                        fig.update_layout(
            ...
            # --- SECCIÓN TABLAS INDIVIDUALES SIN COLUMNA DE LÍNEA GENÉTICA EN LA TABLA ---
            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)
