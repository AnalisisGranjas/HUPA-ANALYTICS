...
if df_orig is not None:
    ...
    if df_l.empty:
        ...
        # Mediano = AA
        # Pequeño = A + B + C
        # Blanco = Alt. Color (Separado)
        # Segunda = Alt Cáscara + Picado + Roto
        ...
        with st.expander("📈 Evolución Temporal del Perfil Comercial de Huevo", expanded=True):
            ...
            with g1:
                # GRÁFICO DE LÍNEAS POR AGRUPACIÓN REVISADA
                fig1 = go.Figure()
                fig1.add_trace(go.Scatter(x=df_plot["Edad Sem."], y=df_plot["Grande %"], mode=mode_graf, name="Grande (Jumbo/Extra)", line=dict(color="#D35400", width=2.5), texttemplate="%{y:.1f}%", textposition="top center"))
                fig1.add_trace(go.Scatter(x=df_plot["Edad Sem."], y=df_plot["Mediano %"], mode=mode_graf, name="Mediano (AA)", line=dict(color="#27AE60", width=2.5), texttemplate="%{y:.1f}%", textposition="top center"))
                fig1.add_trace(go.Scatter(x=df_plot["Edad Sem."], y=df_plot["Pequeño %"], mode=mode_graf, name="Pequeño (A/B/C)", line=dict(color="#2980B9", width=2.5), texttemplate="%{y:.1f}%", textposition="top center"))
                fig1.add_trace(go.Scatter(x=df_plot["Edad Sem."], y=df_plot["Blanco %"], mode=mode_graf, name="Huevo Blanco (Alt. Color)", line=dict(color="#7F8C8D", width=2, dash="dot"), texttemplate="%{y:.1f}%", textposition="top center"))
                fig1.add_trace(go.Scatter(x=df_plot["Edad Sem."], y=df_plot["Segunda %"], mode=mode_graf, name="Segunda / Descarte", line=dict(color="#C0392B", width=2, dash="dash"), texttemplate="%{y:.1f}%", textposition="top center"))
            ...
            with g2:
                # BARRAS APILADAS 100% DE DETALLE COMERCIAL
                fig2 = go.Figure()
                ...
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["Jumbo %"], name="Jumbo", marker_color="#6C3483", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["Extra %"], name="Extra", marker_color="#1F618D", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["AA %"], name="AA", marker_color="#28B463", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["A %"], name="A", marker_color="#F1C40F", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["B %"], name="B", marker_color="#E67E22", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["C %"], name="C", marker_color="#E74C3C", **kwargs_bar))
                fig2.add_trace(go.Bar(x=df_plot["Edad Sem."], y=df_plot["Blanco %"], name="Blanco", marker_color="#BDC3C7", **kwargs_bar))
        ...
        with c_tit:
            st.markdown("### 📋 Matriz Detallada de Clasificación Comercial")
        with c_mod:
