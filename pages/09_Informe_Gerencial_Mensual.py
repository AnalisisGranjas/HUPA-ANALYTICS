...
def load_data():
    if os.path.exists(PATH_DATA):
        try:
            ...
            # Detección de columna Mort (aves muertas)
            col_mort = None
            ...
            # Detección de columna Pdn tabla
            col_pdn_tabla = None
            ...
            num_cols = [
                ...
                "% Pdn. Real", col_pdn_tabla, "Saldo de Aves", col_mort
            ]
...
if df_orig is not None:
    ...
    if df_emp_fechas.empty:
        ...
        # PREPARACIÓN PREVIA Y UNIFICADA DE MATRIZ POR LOTE CON EVALUACIÓN CONTRA PDN TABLA (-2.0% TOLERANCIA)
        res_lotes = []
        alertas_lista = []
        ...
        for (g_name, lt_num), g_df in df_final.groupby(["GRANJA", "LOTE"]):
            ...
            g_df_sorted = g_df.sort_values("Fecha_DT")
            aves_g = g_df_sorted["Saldo de Aves"].iloc[-1]
            edad_g = g_df["Edad Sem."].max()

            # CÁLCULO NÚMERO DE AVES MUERTAS Y % DE MORTALIDAD EN EL PERIODO FILTRADO
            muertas_periodo = g_df[col_mortalidad_num].sum()
            mort_pct_periodo = (muertas_periodo / aves_g * 100.0) if aves_g > 0 else 0.0

            lotes_base_dict[(g_name, lt_num)] = {
                "Granja": g_name,
                "Lote": lt_num,
            ...
            # FÓRMULA IEB CON MORTALIDAD REAL DEL PERIODO (% Viabilidad = 100 - % Mort Periodo)
            ...
            if edad_g > 75:
                lotes_descarte_lista.append({"Granja": g_name, "Lote": lt_num, "Edad": edad_g, "Aves": aves_g})

            pct_segunda = (g_df["Alt Cáscara"] + g_df["Picado"] + g_df["Roto"]).mean() * 100

            # EVALUACIÓN DE MOTIVOS DE ALERTA: COMPARA CONTRA PDN TABLA CON TOLERANCIA DEL 2%
            motivos_alerta = []
            
            if pdn_tabla_g > 0:
                diferencia_pdn = pdn_g - pdn_tabla_g
                if diferencia_pdn < -2.0:
                    motivos_alerta.append(f"Postura {pdn_g:.1f}% por debajo de tabla {pdn_tabla_g:.1f}% (Brecha: {diferencia_pdn:.1f}%)")
            else:
                if pdn_g < 80.0:
            ...
            # DICCIONARIO BASE DE LOTE: SE UBICA LA MORTALIDAD DEBAJO DE AVES ACTIVAS Y EN NÚMERO DE AVES
            item_lote = {
                "Granja": g_name,
                "Lote": f"Lote {lt_num}",
                "Edad_Num": edad_g,
                ...
                "Aves_Num": aves_g,
                "Aves Activas (Censo Final)": f"{aves_g:,.0f}",
                "Mortalidad (Aves Muertas)": f"{muertas_periodo:,.0f} Aves",
            ...
            # SOLO SI ES ADMIN SE AGREGA EL CAMPO DE COSTO
            if es_admin:
        ...
        # CÁLCULOS PONDERADOS POR AVES (CENSO) Y TOTALES AUDITADOS
        ...
        if total_aves_sum > 0:
            ...
            mort_pct_global = (total_muertas_sum / total_aves_sum) * 100.0
        else:
        ...
        # IEB Promedio Ponderado General
        ...
        with st.container():
            ...
            # KPIS GENERALES (SI NO ES ADMIN, MUESTRA MORTALIDAD EN VEZ DE COSTO)
            ...
            fila_totales = {
                ...
                "Aves Activas (Censo Final)": f"{total_aves_sum:,.0f} (Total Censo)",
                "Mortalidad (Aves Muertas)": f"{total_muertas_sum:,.0f} Aves ({mort_pct_global:.1f}%)",
                "Fase Alimento": "Consolidado",
                "Consumo (Ton)": f"{total_ton_sum:,.1f} Ton",
                "% Pdn. Real": f"{pdn_prom:.1f}% (Prom)",
            ...
            if es_admin:
                fila_totales["$/Huevo x Alimento"] = f"${costo_prom_huevo:.1f} (Prom)"
                cols_display = ["Granja", "Lote", "Edad Sem.", "Línea Genética", "Aves Activas (Censo Final)", "Mortalidad (Aves Muertas)", "Fase Alimento", "Consumo (Ton)", "% Pdn. Real", "$/Huevo x Alimento", "Conversión (g)", "IEB (Pts)", "Estado"]
            else:
                cols_display = ["Granja", "Lote", "Edad Sem.", "Línea Genética", "Aves Activas (Censo Final)", "Mortalidad (Aves Muertas)", "Fase Alimento", "Consumo (Ton)", "% Pdn. Real", "Conversión (g)", "IEB (Pts)", "Estado"]
            ...
            # --- NOTA EXPLICATIVA SÚPER AMIGABLE EN LA APP ---
            ...
            # --- SECCIÓN 3: DIAGNÓSTICO TÉCNICO Y PLANES DE RENUEVO DE FLOTA ---
            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)
            ...
            with c_audit1:
                ...
                for a in alertas_lista:
                    st.markdown(f"<li><b>{a['Granja']} - Lote {a['Lote']}</b> ({a['Edad']}): <span style='color:#C0392B;'>{a['Problemas Detectados']}</span></li>", unsafe_allow_html=True)
            ...
            with c_audit2:
                ...
                if lotes_descarte_lista:
                    for d in lotes_descarte_lista:
                        st.markdown(f"• 🔴 <b>{d['Granja']} - Lote {d['Lote']}</b>: {d['Edad']:.0f} Semanas | Censo: <b>{d['Aves']:,.0f} aves</b>.<br>", unsafe_allow_html=True)
                else:
        ...
        # GENERADOR HTML PDF DINÁMICO SEGÚN ROL
        def generar_pdf_html(emp_nombre, df_ordered, t2_data, alertas, descarte_lotes, tot_row, c_prim, c_sec, logo_tag, ton_tot, df_gen_pct, df_fase_pct, logo_b64_str, pdn_global_val, es_adm):
            ...
            for _, r in df_ordered.iterrows():
                ...
                rows_1 += f"<tr><td>{r['Granja']}</td><td>{r['Lote']}</td><td>{r['Edad Sem.']}</td><td>{r['Línea Genética']}</td><td>{r['Aves Activas (Censo Final)']}</td><td>{r['Mortalidad (Aves Muertas)']}</td><td>{r['Fase Alimento']}</td><td>{r['Consumo (Ton)']}</td><td>{r['% Pdn. Real']}</td>{val_costo_td}<td>{r['Conversión (g)']}</td><td>{r['IEB (Pts)']}</td><td>{r['Estado']}</td></tr>"
            
            val_costo_tot_td = f"<td>{tot_row['$/Huevo x Alimento']}</td>" if es_adm else ""
            row_tot_html = f"<tr style='background-color:#FDF2E9; font-weight:bold; border-top:2px solid {c_prim};'><td>{tot_row['Granja']}</td><td>{tot_row['Lote']}</td><td>{tot_row['Edad Sem.']}</td><td>{tot_row['Línea Genética']}</td><td>{tot_row['Aves Activas (Censo Final)']}</td><td>{tot_row['Mortalidad (Aves Muertas)']}</td><td>{tot_row['Fase Alimento']}</td><td>{tot_row['Consumo (Ton)']}</td><td>{tot_row['% Pdn. Real']}</td>{val_costo_tot_td}<td>{tot_row['Conversión (g)']}</td><td>{tot_row['IEB (Pts)']}</td><td>{tot_row['Estado']}</td></tr>"
            ...
            p_text = "".join([f"<li><b>{a['Granja']} - Lote {a['Lote']}</b> ({a['Edad']}): <span style='color:#C0392B;'>{a['Problemas Detectados']}</span></li>" for a in alertas]) if alertas else "<li>No se registraron desviaciones en el periodo.</li>"

            d_text = "".join([f"<li>🔴 <b>{d['Granja']} - Lote {d['Lote']}</b> ({d['Edad']:.0f} Sem): Censo de <b>{d['Aves']:,.0f} aves</b>.</li>" for d in descarte_lotes]) if descarte_lotes else "<li>No hay lotes en zona de descarte crítico (>75 Sem).</li>"
...
<head>
    ...
    <style>
        ...
        table {{ width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 10.5px; page-break-inside: auto; }}
        ...
        th {{ background: {c_prim}; color: white; padding: 6px; text-align: center; font-size: 10.5px; }}
        td {{ padding: 5.5px; border-bottom: 1px solid #EEE; text-align: center; }}
...
<body onload="window.print()">
    ...
    <table>
        <thead>
            <tr><th>Granja</th><th>Lote</th><th>Edad Sem.</th><th>Línea Genética</th><th>Aves (Censo Final)</th><th>Mortalidad (Bajas)</th><th>Fase</th><th>Consumo</th><th>Pdn %</th>{th_costo_col}<th>Conversión</th><th>IEB</th><th>Estado</th></tr>
        </thead>
