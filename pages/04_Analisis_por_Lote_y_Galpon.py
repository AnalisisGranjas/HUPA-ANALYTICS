...
if df_orig is not None:
    ...
    st.markdown('<div class="main-title">🚀 ANALISIS DE LOTE POR GALPONES</div>', unsafe_allow_html=True)
    ...
    #st.markdown(f"""
        #<div class="intro-card" style="background-color: #e3f2fd; padding: 30px; border-radius: 15px; border-left: 10px solid #0d47a1; margin-bottom: 25px;">
            ...
            #<p style="text-align: justify; font-size: 1.05rem; line-height: 1.6; color: #1a237e;">
                #<b>¿Qué estamos analizando?</b> Realizamos una disección técnica de un Lote Biológico para comparar el comportamiento individual de sus Galpones. Desglosamos la "masa" de datos para encontrar ineficiencias que el promedio general del lote suele ocultar.<br><br>
                #<b>¿Cómo lo hacemos?</b> Mediante algoritmos de correlación, comparamos cada galpón contra su propia meta genética y contra sus "hermanos". Evaluamos la tríada maestra: <b>Producción de Huevo, Dinámica de Consumo y Tasa de Mortalidad.</b><br><br>
                #<b>¿Por qué es vital?</b> En lotes de la misma edad, genética y nutrición, cualquier diferencia en resultados es una <b>auditoría directa al manejo</b>. Si un galpón rinde menos, el problema no es el ave ni el alimento; es el ambiente, el equipo o el protocolo operativo de esa galpón específico.<br><br>
                #<b>Importancia:</b> La uniformidad entre galpones es el indicador más puro de <b>Rentabilidad Real</b>. Detectar un desvío a tiempo permite corregir fallas en producción, desperdicio de alimento o mortalidad antes de que se conviertan en pérdidas económicas irreversibles.
            #</p>
        #</div>
    #""", unsafe_allow_html=True)
    ...
    with cx2: ver_meta = st.toggle("📉 Ver Tabla", value=True)
    ...
    if df_filtered.empty:
        ...
        rango_edad = st.slider("⏳ Ventana de Análisis (Semanas):", min_e, max_e, (max(min_e, max_e-4), max_e))
...
# --- SECCIÓN A: ASISTENCIA TÉCNICA IA (DIAGNÓSTICO ESTRATÉGICO) ---
        # 1. El título del expander debe ser texto plano. Le añadimos expanded=False para que inicie cerrado.
        with st.expander("🤖 ASISTENCIA TÉCNICA ESTRATÉGICA IA - Clic para desglosar", expanded=False):
            
            # 2. TODO el código de abajo tiene que estar indentado hacia la derecha
            ...
            if not ultimo_corte.empty:
                ...
                for _, row in ultimo_corte.iterrows():
                    ...
                    # Asumo que 'Conv' lo usas en otra parte, así que lo dejamos como lo tienes
                    conv = row.get('Conv', 0) 
        ...
        def plot_tecnico(df, y_col, tab_col, title, unit, labels, meta_line):
            ...
            if meta_line and tab_col in df.columns:
                ...
                fig.add_scatter(x=df_m['Edad Sem.'], y=df_m[tab_col], mode='lines', line=dict(color='grey', dash='dot', width=2), name='Meta Genética')
            fig.update_layout(title=f"<b>{title}</b>", title_x=0.5, height=400, hovermode="x unified", legend=dict(orientation="h", y=-0.2))
            fig.update_traces(
        # Si 'labels' es True, muestra líneas+puntos+texto. Si es False, solo líneas+puntos.
        mode="lines+markers+text" if labels else "lines+markers",
        texttemplate="%{y:.1f}" if labels else None, 
        textposition="top center",
        hovertemplate="Semana %{x}<br>Valor: %{y:.1f}" + unit + "<extra></extra>"
            )
            return fig
        ...
        c3, c4 = st.columns(2)
        with c3: st.plotly_chart(plot_tecnico(df_view, '% Mort + Sel Acum.', '%Mort+Sel Acum. Tab', "⚰️ MORTALIDAD ACUMULADA (%)", "%", ver_etiquetas, ver_meta), use_container_width=True)
        with c4: st.plotly_chart(plot_tecnico(df_view, 'Peso Real', 'Peso Tab', "⚖️PESO CORPORAL (GRAMOS)", "g", ver_etiquetas, ver_meta), use_container_width=True)
        ...
        'Costo Alimento Sem': '${:,.0f}', '$ Huevo por alimento': '${:,.1f}'
                ...
                st.dataframe(df_tab[present_cols].fillna(0).style.format(formatos).map(
