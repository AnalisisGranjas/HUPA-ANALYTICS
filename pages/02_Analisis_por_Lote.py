...
if df_orig is not None:
    ...
    if not df_full_lote.empty:
        ...
        st.dataframe(tabla_final.style.format({k: v for k, v in formatos.items() if k in tabla_final.columns}).applymap(style_diferencias, subset=[c for c in ['Dif Pdn', 'Dif GAD', 'Dif HAA', 'Dif Peso', 'Dif Mort'] if c in tabla_final.columns]), use_container_width=True, hide_index=True)
