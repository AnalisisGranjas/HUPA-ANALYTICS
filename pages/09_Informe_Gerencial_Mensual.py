...
if df_orig is not None:
    ...
    if df_emp_fechas.empty:
        st.warning(f"⚠️ No se encontraron registros coincidentes (donde LOTE == GALPON) para **{empresa_sel}** en el rango ({txt_periodo}).")
        ...
        # DETECCIÓN DE COLUMNA ARCHIVO ORIGEN (SOLO PARA AGRUPAR EN BACKGROUND)
        col_archivo = None
        for ca in ["ARCHIVO_ORIGEN", "Archivo_Origen", "archivo_origen", "ARCHIVO", "Archivo", "archivo"]:
            if ca in df_final.columns:
                col_archivo = ca
                break
        ...
        for group_keys, g_df in df_final.groupby(group_cols):
            ...
            lote_label_display = f"Lote {lt_num}"
            ...
            # CENSO EXACTO EN LA ÚLTIMA FECHA
            ...
            if edad_g > 75:
                lotes_descarte_lista.append({"Granja": g_name, "Lote": lote_label_display, "Edad": edad_g, "Aves": aves_g})
        ...
        with st.container():
            ...
            # COLUMNAS A MOSTRAR: LIMPIO SIN ARCHIVO NI GALPÓN
            if es_admin:
