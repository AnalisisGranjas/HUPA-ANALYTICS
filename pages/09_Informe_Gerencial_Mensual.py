...
if df_orig is not None:
    ...
    if df_emp_fechas.empty:
        ...
        for group_keys, g_df in df_final.groupby(group_cols):
            ...
            # --- CORRECCIÓN: CÁLCULO DE ESTADO Y ALERTAS BASADO ESTRICTAMENTE EN IEB ---
            motivos_alerta = []
            
            # 1. Asignación del Estado según el IEB
            if ieb_score >= 85.0:
                estado_lote = "✅ Óptimo"
            elif ieb_score >= 75.0:
                estado_lote = "⚠️ Revisa Atención"
            else:
                estado_lote = "🔴 Alerta Crítica"

            # 2. Documentar las causas si el lote requiere revisión (< 85 Pts)
            if ieb_score < 85.0:
                if pdn_tabla_g > 0 and dif_pdn_val < -2.0:
                ...
                if not motivos_alerta:
                    motivos_alerta.append(f"Índice de Eficiencia Biológica bajo ({ieb_score:.1f} Pts)")
            ...
            item_lote = {
                ...
                "Aves_Num": aves_g,
                "Saldo de Aves": f"{aves_g:,.0f}",
                "Mort": f"{muertas_periodo:,.0f}",
                "CASA NUTRICIONAL": str_fases,
                "Toneladas_Num": ton_lote,
                "Consumo (Ton)": f"{ton_lote:,.1f} Ton",
                "Pdn_Num": pdn_g,
                "% Pdn. Real": f"{pdn_g:.1f}%",
                "% Pdn. Tabla": f"{pdn_tabla_g:.1f}%",
                "Dif Pdn": f"{dif_pdn_val:+.1f}%",
