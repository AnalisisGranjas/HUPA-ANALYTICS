...
# --- 3. CARGA DE DATOS ALINEADA Y LIMPIA ---
...
def load_data():
    if os.path.exists(PATH_DATA):
        ...
        num_cols = [
            ...
            "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "H.A.A. Real", "H.A.A. Tabla",
            "Peso Real", "Peso Tab", "Saldo de Aves", "Bulto X 40 K", "Huevos  Semana"
        ]
...
if df_orig is not None:
    with st.expander("🔍 Protocolo de Fiscalización Operativa e Instrucciones", expanded=False):
        st.markdown(
            """
    ...
    with c2:
        granjas_disp = sorted(df_orig[df_orig["RAZON_SOCIAL"] == empresa]["GRANJA"].unique())
    ...
    for ca in ["ARCHIVO_ORIGEN", "Archivo_Origen", "archivo_origen", "ARCHIVO", "Archivo", "archivo"]:
        if ca in df_activos.columns:
            col_archivo = ca
            break

    # FILTRAR REGISTROS DEL LOTE SELECCIONADO
    df_lote_raw = df_activos[df_activos["LOTE"] == lote_sel].copy()

    # SI HAY MÚLTIPLES ARCHIVOS ORIGEN PARA EL MISMO LOTE, CONSOLIDAR HISTÓRICO POR EDAD
    if col_archivo:
        group_hist_cols = ["Edad Sem."]
        df_full_lote_list = []
        for sem, sem_df in df_lote_raw.groupby("Edad Sem."):
            row_sem = sem_df.iloc[-1].copy()
            
            # Sumar Censo y Producción de todos los galpones en esa semana
            tot_aves = sem_df["Saldo de Aves"].sum()
            row_sem["Saldo de Aves"] = tot_aves
            row_sem["Huevos  Semana"] = sem_df["Huevos  Semana"].sum() if "Huevos  Semana" in sem_df.columns else 0
            if "Mort" in sem_df.columns: row_sem["Mort"] = sem_df["Mort"].sum()
            if "Bulto X 40 K" in sem_df.columns: row_sem["Bulto X 40 K"] = sem_df["Bulto X 40 K"].sum()
            
            # Promedios Ponderados por aves para la postura
            if tot_aves > 0:
                row_sem["% Pdn. Real"] = (sem_df["% Pdn. Real"] * sem_df["Saldo de Aves"]).sum() / tot_aves
                row_sem["% Pdn. Tabla"] = (sem_df["% Pdn. Tabla"] * sem_df["Saldo de Aves"]).sum() / tot_aves
                row_sem["% Mort + Sel Acum."] = (sem_df["% Mort + Sel Acum."] * sem_df["Saldo de Aves"]).sum() / tot_aves
                row_sem["%Mort+Sel Acum. Tab"] = (sem_df["%Mort+Sel Acum. Tab"] * sem_df["Saldo de Aves"]).sum() / tot_aves
        ...
        df_full_lote = pd.DataFrame(df_full_lote_list).sort_values("Edad Sem.")
    else:
        df_full_lote = df_lote_raw.sort_values("Edad Sem.").copy()
    ...
    if not df_full_lote.empty:
        ...
        def render_kpi(col, title, emoji, val, tab, ant_val, unit, is_mort=False):
            sub_tab = ""
            if tab is not None:
                dif = val - tab
                color_sub = (
            ...
            if title == "SALDO AVES":
                sub_ant = f'<span style="opacity:0.8;">Alojadas: {saldo_inicial:,.0f}</span>'
                sub_tab = f'<span style="opacity:0.8;">Mort: {aves_perdidas:,.0f}</span>'
        ...
        render_kpi(k_cols[0], "PRODUCCIÓN", "🥚", ult["% Pdn. Real"], ult["% Pdn. Tabla"], ant["% Pdn. Real"], "%")
        render_kpi(k_cols[1], "CONSUMO", "🥣", ult["Gr.A.D Real"], ult["Gr.A.D Tabla"], ant["Gr.A.D Real"], "g")
        render_kpi(k_cols[2], "MORTALIDAD ACUM", "💀", ult["% Mort + Sel Acum."], ult["%Mort+Sel Acum. Tab"], ant["% Mort + Sel Acum."], "%", is_mort=True)
        render_kpi(k_cols[3], "H.A.A", "🏆", ult["H.A.A. Real"], ult["H.A.A. Tabla"], ant["H.A.A. Real"], "")
        render_kpi(k_cols[4], "SALDO AVES", "🐥", ult["Saldo de Aves"], None, None, "")
        ...
        with st.expander("📊 Ver Curvas de Comportamiento Técnico", expanded=True):
            def plot_hupa_final(df_current, df_total, real_col, tab_col, title, unit):
                ...
                if tab_col and tab_col in df_plot.columns:
                    ...
                    htemp = (
                        ...
                        "Real: %{y:.1f}" + unit +
                        "<br>Tabla: %{customdata[0]:.1f}" + unit +
                        "<br>Diferencia: <b>%{customdata[1]:+.1f}</b>" + unit +
                        "<extra></extra>"
                    )
                    ...
                    if semanas_ia is not None:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=valores_ia[mask],
                                name="🤖 IA Proyección",
                                line=dict(color="#17A2B8", width=2.5, dash="dash"),
                                mode="lines",
                ...
                fig.update_layout(
                    title={"text": f"<b>{title}</b>", "x": 0.5, "xanchor": "center", "font": dict(size=14)},
                    hovermode="x unified",
            ...
            g1, g2 = st.columns(2)
            with g1:
                fig1 = plot_hupa_final(df_lote, df_full_lote, "% Pdn. Real", "% Pdn. Tabla", "🥚 POSTURA REAL VS TABLA (%)", "%")
                st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Mide la eficiencia en la curva de postura. Una caída sostenida bajo la guía requiere revisión.</div>', unsafe_allow_html=True)

            with g2:
                fig2 = plot_hupa_final(df_lote, df_full_lote, "Gr.A.D Real", "Gr.A.D Tabla", "🌽 CONSUMO DE ALIMENTO (G/AVE/DÍA)", "g")
                st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Mide consumo de alimento acumulado y diario frente a la guía genética.</div>', unsafe_allow_html=True)

            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)

            g3, g4 = st.columns(2)
            with g3:
                fig3 = plot_hupa_final(df_lote, df_full_lote, "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "💀 MORTALIDAD ACUMULADA (%)", "%")
                st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Supervivencia del lote encasetado vs. tabla guía.</div>', unsafe_allow_html=True)

            with g4:
                fig4 = plot_hupa_final(df_lote, df_full_lote, "Peso Real", "Peso Tab", "🐤 PESO CORPORAL (G)", "g")
                st.plotly_chart(fig4, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Evolución del peso corporativo promedio del lote.</div>', unsafe_allow_html=True)

            st.markdown('<div class="hr-custom"></div>', unsafe_allow_html=True)

            g5, g6 = st.columns(2)
            with g5:
                fig5 = plot_hupa_final(df_lote, df_full_lote, "H.A.A. Real", "H.A.A. Tabla", "🏆 HUEVOS POR AVE ALOJADA (H.A.A.)", "")
                st.plotly_chart(fig5, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Sumatoria acumulada de huevos producidos por ave.</div>', unsafe_allow_html=True)

            with g6:
                fig6 = plot_hupa_final(df_lote, df_full_lote, "Conversión", None, "💰 CONVERSIÓN (G ALIMENTO / HUEVO)", "g")
                st.plotly_chart(fig6, use_container_width=True, config={"displayModeBar": False})
                st.markdown('<div class="guide-box"><b>Observaciones:</b> Eficiencia de conversión alimenticia.</div>', unsafe_allow_html=True)
        ...
        with c_tit:
            st.markdown(
                f"### 📋 Historial Detallado del Lote <span class=\"genetica-badge\">🧬 Genética: {genetica_actual}</span>",
                unsafe_allow_html=True,
            )
        with c_mod:
        ...
        cols_base = [
            "Final Sem", "Edad Sem.", "Saldo de Aves", "Mort", "Suma Mort + Sel",
            "% Mort + Sel Acum.", "%Mort+Sel Acum. Tab", "Dif Mort", "Fase de Alimento",
            "Observaciones", "Bulto X 40 K", "Huevos  Semana", "Huevos Faltantes Sem.",
            "Gr.A.D Real", "Gr.A.D Tabla", "Dif GAD", "% Pdn. Real", "% Pdn. Tabla",
            "Dif Pdn", "% Unif", "Peso Real", "Peso Tab", "Dif Peso", "H.A.A. Real",
            "H.A.A. Tabla", "Dif HAA", "Costo Alimento Sem", "$ Huevo por alimento",
        ]
        ...
        with st.popover("⚙️ Seleccionar Columnas Visibles"):
            cols_visibles = [col for col in cols_disponibles if st.checkbox(col, value=True, key=f"hist_vis_{col}")]

        if not cols_visibles:
            cols_visibles = cols_disponibles

        df_tabla_source = df_lote if "df_lote" in locals() and not df_lote.empty else df_full_lote
        tabla_final = df_tabla_source[cols_visibles].sort_values("Edad Sem.", ascending=False).copy()
        ...
        with st.expander("📥 Descargar Reporte del Lote (Excel)"):
            st.download_button(
                ...
                file_name=f"Reporte_Lote_{lote_sel}_{datetime.datetime.now().strftime('%Y%m%d')}.xlsx",
        ...
        try:
            ...
            st.markdown(f"<div style='text-align: right; opacity: 0.6; font-size: 0.85rem; font-style: italic; margin-top: 10px;'>📅 Última actualización: {fecha_act}</div>", unsafe_allow_html=True)
        except:
            pass
