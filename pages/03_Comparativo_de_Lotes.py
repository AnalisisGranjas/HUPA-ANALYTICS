...
#""", unsafe_allow_html=True)
    ...
    with c1:
        # Filtro de Empresa
        with st.popover("🏢 1. Empresa:"):
            empresa_sel = st.multiselect(
                "Seleccionar Empresa:", 
                options=sorted(df_activos['RAZON_SOCIAL'].unique()),
                default=sorted(df_activos['RAZON_SOCIAL'].unique()),
                label_visibility="collapsed"
            )
    with c2:
        # Filtro de Granja (Dependiente de Empresa)
        df_g = df_activos[df_activos['RAZON_SOCIAL'].isin(empresa_sel)] if empresa_sel else df_activos
        with st.popover("🗺️ 2. Granja(s):"):
            granja_sel = st.multiselect(
                "Seleccionar Granja:", 
                options=sorted(df_g['GRANJA'].unique()),
                label_visibility="collapsed"
            )
    with c3:
        # Filtro de Lote (Dependiente de Granja)
        df_l = df_g[df_g['GRANJA'].isin(granja_sel)] if granja_sel else df_g
        with st.popover("📦 3. Lote(s):"):
            lote_sel = st.multiselect(
                "Seleccionar Lote:", 
                options=sorted(df_l['LOTE'].unique()),
                label_visibility="collapsed"
            )    
    with c4:
        # Filtro de Casa Nutricional
        opciones_casa = sorted([str(x) for x in df_l['Observaciones'].unique() if pd.notna(x)]) if not df_l.empty else []
        with st.popover("🌽 4. Casa Nutricional:"):
            casa_sel = st.multiselect(
                "Seleccionar Casa:", 
                options=opciones_casa,
                label_visibility="collapsed"
            )
        ...
        def plot_hupa(df, y_col, tab_col, title, unit, show_labels, show_table):
            ...
            if show_table and tab_col:
                ...
                fig.add_scatter(x=df_tab['Edad Sem.'], y=df_tab[tab_col], mode='lines', 
                                line=dict(color='grey', dash='dot', width=2), name='Meta Ideal')
            ...
            with tab:
                ...
                def style_diferencias(val):
                    try:
                        ...
                        color = '#27ae60' if num > 0 else '#e74c3c' if num < 0 else '#636e72'
                        return f'color: {color}; font-weight: bold;'
                    except: return ''

                # Preparamos la tabla (reseteando índice para evitar la columna fea)
                tabla_para_presentar = tabla_render.reset_index(drop=True)
                formatos_activos = {k: v for k, v in formatos.items() if k in tabla_para_presentar.columns}

                tabla_estilizada = tabla_para_presentar.style \
                    .format(formatos_activos) \
                ...
                if modo_proyector:
                    st.markdown("""
                        ...
                        .row_heading, .blank { display: none !important; }
                        th.index_name { display: none !important; }
                    ...
                    <style>
                        ...
                        /* 2. Color para la esquina superior izquierda */
